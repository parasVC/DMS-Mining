...
const Filter = ({ filterType }: { filterType: string }) => {
    ...
    return (
        <div className="flex gap-4 w-full">
            ...
            {filters.input && filters.input.length > 0 && (filters.input.map((ele, i) =>
                <div key={`filter-${ele.key}-${i}`} className='flex flex-col gap-3 w-full'>
                    <Label>{ele.label}</Label>
                    <Input defaultValue={searchParams.get(ele.key ?? '') || ''} onBlur={(e) => updateFilter(ele.key, e.target.value)} />
                </div>
            ))}
                    ...
                    <Popover>
                        <PopoverTrigger asChild>
                            <Button variant="outline" className="w-full justify-start text-left">
                                ...
                                {createdAtValue ? format(parseISO(createdAtValue), "yyyy-MM-dd") : "Select Date"}
                            </Button>
                        </PopoverTrigger>
                            ...
                            <Calendar
                                ...
                                onSelect={(date) => {
                                    if (filters.created_at?.key) {
                                        updateFilter(filters.created_at?.key, date ? format(date, "yyyy-MM-dd") : "")
                                    }
                                }
                                }
                            />
            ...
            {/* Status Filter */}
            {filters.select && filters.select.length > 0 &&
                        ...
                        >
                            ...
                            <SelectContent>
                                {selectItem.fields.map((field, i) => (
                                    <SelectItem key={`filter-${field.label}-${i}`} value={field.defaultValue}>
                                        {field.label}
                                    </SelectItem>
                                ))}
                            </SelectContent>
