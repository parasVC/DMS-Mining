...
export default function LicenseTable({ data = [], page, totalPages }: TableComponentProps) {
    ...
    const filters = {
        [FIELD_PARAMS.LICENSE_NUMBER]: searchParams.get(FIELD_PARAMS.LICENSE_NUMBER) || "",
        [FIELD_PARAMS.CREATED_AT]: searchParams.get(FIELD_PARAMS.CREATED_AT) || "",
        [FIELD_PARAMS.ASSIGNED_STATUS]: searchParams.get(FIELD_PARAMS.ASSIGNED_STATUS) || "",
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-5 gap-4">
                <div>
                    ...
                    <Input placeholder="License Number" defaultValue={filters[FIELD_PARAMS.LICENSE_NUMBER]} onBlur={(e) => updateFilter(FIELD_PARAMS.LICENSE_NUMBER, e.target.value)} />
                </div>
                    ...
                    <Popover>
                        <PopoverTrigger asChild>
                            <Button variant="outline" className="w-full justify-start text-left">
                                ...
                                {filters[FIELD_PARAMS.CREATED_AT] ? format(parseISO(filters[FIELD_PARAMS.CREATED_AT]), "yyyy-MM-dd") : "Select Date"}
                            </Button>
                        </PopoverTrigger>
                        <PopoverContent align="start" className="w-auto p-0">
                            <Calendar
                                mode="single"
                                selected={filters[FIELD_PARAMS.CREATED_AT] ? parseISO(filters[FIELD_PARAMS.CREATED_AT]) : undefined}
                                onSelect={(date) => updateFilter(FIELD_PARAMS.CREATED_AT, date ? format(date, "yyyy-MM-dd") : "")}
                            />
                        </PopoverContent>
                    </Popover>
                </div>
                ...
                <div>
                    <Label>Status</Label>
                    <Select defaultValue={filters.status || "all"} onValueChange={(value) => updateFilter("status", value)}>
                ...
                <div>
                    ...
                    <Select defaultValue={filters[FIELD_PARAMS.ASSIGNED_STATUS] || "all"} onValueChange={(value) => updateFilter(FIELD_PARAMS.ASSIGNED_STATUS, value)}>
                        <SelectTrigger className="w-full">
                            <SelectValue placeholder="Status" />
                        </SelectTrigger>
                        <SelectContent>
                                ...
                                {columns.map((col) => (
                                    <TableCell key={`cell-${item.id}-${col.key}-${index}`}>
                                        {col.key === "actions" ? (
                                            ...
                                            <Badge className={item[FIELD_PARAMS.LICENSE_STATUS] === "ACTIVE" ? "bg-[#84CC16] text-white" : "bg-red-500 text-white"}>
                                                {item[FIELD_PARAMS.LICENSE_STATUS] === "ACTIVE" ? "Active" : "Not Active"}
                                            </Badge>
                                        ) :
                                            col.key === FIELD_PARAMS.IS_ASSIGNED ? ( // Check if column is "status"
                                                <Badge className={item[FIELD_PARAMS.IS_ASSIGNED] ? "bg-[#84CC16] text-white" : "bg-[#FFC130] text-white"}>
                                                    {item[FIELD_PARAMS.IS_ASSIGNED] ? "Assign" : "Not Assign"}
                                                </Badge>

                                            ) : col.format ? col.format(item[col.key]) : (item[col.key as keyof DataItem] as string)}
                                    </TableCell>
                                ))}
                            </TableRow>
                        ))
