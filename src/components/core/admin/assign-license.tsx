...
export default function AssignLicense({
    ...
    const form = useForm<coreFormData>({
        ...
        defaultValues: { [FIELD_PARAMS.NUM_LICENSES]: "", [FIELD_PARAMS.EXPIRY_DATE]: "" },
    });
    ...
    const handleDateSelection = (months: number) => {
        const newDate = months === 12 ? addYears(new Date(), 1) : addMonths(new Date(), months);
        setValue(FIELD_PARAMS.EXPIRY_DATE, format(newDate, "yyyy-MM-dd"), { shouldValidate: true });
    };
    ...
    return (
        ...
        <Popup
            ...
            trigger={<Button className="p-2 flex gap-3 justify-start items-center" variant={"ghost"} onClick={() => setIsOpen(true)}><Crown size={16} /><span className="text-sm">Assign licenses</span></Button>}
                ...
                <Form {...form}>
                    <form onSubmit={handleSubmit(handleSubmitForm)} className="space-y-4">
                                        ...
                                        <PopoverContent className="w-auto p-0">
                                            <Calendar
                                                ...
                                                onSelect={(selectedDate) => {
                                                    if (selectedDate) {
                                                        setValue(FIELD_PARAMS.EXPIRY_DATE, format(selectedDate, "yyyy-MM-dd"), { shouldValidate: true });
                                                    }
                                                }}
                                                disabled={(day) => day < new Date()} // Disable past dates
                                            />
                                        </PopoverContent>
                                    </Popover>
                                    <FormMessage />
                        ...
                        <div className="flex justify-end text-blue-600 text-sm underline">
                            <Button type="button" variant={"ghost"} onClick={() => handleDateSelection(3)}>3 Month</Button>
                            <Button type="button" variant={"ghost"} onClick={() => handleDateSelection(6)}>6 Month</Button>
                            <Button type="button" variant={"ghost"} onClick={() => handleDateSelection(12)}>1 Year</Button>
                        </div>
