...
import { CalendarDays, CircleChevronRight, CircleDollarSign } from 'lucide-react'
        ...
        <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {seedList.length > 0 ? (seedList.map((report) => (
                    <Card onClick={() => { router.push(`/university/report?seed_id=${report.seed_id}&user_id=${userData.id}&student_name=${userData.first_name} ${userData.last_name}`) }} key={report.seed_id} className=" flex justify-between items-center p-5 gap-3 cursor-pointer rounded-sm">
                        <div>
                            <CardTitle className="text-xl font-medium">Seed No: {report.seed_id}</CardTitle>
