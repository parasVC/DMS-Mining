...
export default function CreateSeed() {
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmitForm)} className="flex flex-col gap-2">
                <FormLabel>Seed Generation</FormLabel>
                <div className="flex gap-2">
                    ...
                    />
                    <Button className="p-1 items-center" type="submit" disabled={form.formState.isSubmitting} variant={"outline"}>
                        {form.formState.isSubmitting ? <LoaderCircle className="mr-2 size-4 animate-spin" /> : <PlusCircle />}<span className="text-sm">Add New</span>
                    </Button>
                </div>
            </form>
        </Form>
